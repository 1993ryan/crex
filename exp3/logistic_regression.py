...
for Name in ['bert',  'trex', 'word2vec']:
    ...
    auc=cross_val_score(CLF_LR, Features, Labels, cv=10, scoring='roc_auc').mean()
    # print(recall1)
    print(recall)
    print(acc)
    print(prec)
    ...
    df["fp"] = maskfp
    df["fn"] = maskfn
    df.to_csv("./dataout/"+Name+"202109031111.结果.csv")

    pd.DataFrame(df.loc[masktp,:].index).to_csv("./dataout/"+Name+"tp.csv")
    pd.DataFrame(df.loc[masktn, :].index).to_csv("./dataout/"+Name + "tn.csv")
    pd.DataFrame(df.loc[maskfp, :].index).to_csv("./dataout/"+Name + "fp.csv")
    pd.DataFrame(df.loc[maskfn, :].index).to_csv("./dataout/"+Name + "fn.csv")
